...
#' @param expr A numeric matrix of gene expression matrix. Each row is a gene and each column is a cell. Row names are gene names and column names are cell names. Cells from all samples should be included in this matrix.
#' @param sample A character vector indicating which sample each cell comes from. Need to correspond with column names in \code{expr}.
#' @param testtype One of 'paired', 'unpaired', 'continuous', and 'custom'. See details and examples.
#' @param design Format depends on \code{testtype}.
#' \itemize{
...
#'  }
#' @param intercept TRUE or FALSE. TRUE: add the intercept to the design matrix. FALSE: use the original design matrix without additional intercept. This should be set to be TRUE in most cases if no intercept already exists in design matrix. Only useful if \code{testtype} is not 'custom'.
#' @param filtergene If TRUE, filter out genes with low expression. Pseudobulks will be calculated by averaging the expression across cells within each sample. A gene is retained if there is at least one sample with pseudobulk expression larger than a cutoff. The cutoff is the \code{filtergenequantile} quantile of pseudobulks across all genes and samples.
#' @param filtergenequantile Only useful if \code{filtergene} is TRUE. Used to determine the cutoff for gene filtering. See \code{filtergene}.
#' @param ncores Number of cores to be used in the fitting.
#' @details RAISIN provides four types of fitting specified by the \code{testtype} argument: 'paired', 'unpaired', 'continuous', and 'custom'.
#' \itemize{
...
#' }
#' @import limma parallel statmod Matrix
#' @export
#' @return A list of elements used for raisintest.
...
raisinfit <- function(expr,sample,testtype,design,intercept=TRUE,filtergene=FALSE,filtergenequantile=0.5,ncores=detectCores()) {
  # presettings
  ...
  gaussquad <- gauss.quad(1000,"laguerre")
